	...
	if parent.stateDead == false:
		...
		if seek_timer.is_stopped():
			if leftclose.overlaps_body(player):
				speed = 150 + (variation*5)
				parent.velocity.x = -speed
			elif rightclose.overlaps_body(player):
				speed = 150 + (variation*5)
				parent.velocity.x = speed
			elif player.global_position.x < left_limit.global_position.x and not leftclose.overlaps_body(player):
				speed = 350 + (variation*10)
				parent.velocity.x = -speed
			elif player.global_position.x > right_limit.global_position.x and not rightclose.overlaps_body(player):
				speed = 350 + (variation*10)
				parent.velocity.x = speed
				
			if player.global_position.y > self.global_position.y:
				speed = 150 + (variation*10)
